...
/**
 * Verifica se o aniversário está próximo (dentro de X dias)
 * e retorna um array com info útil para exibição.
 */
function birthday_info(?string $dataNascimento, int $windowDays = 7): array {
    ...
    try {
        ...
        // Formata data de exibição: "dd/mm"
...
/* ===========================
   GARANTIR COLUNA data_nascimento
   =========================== */
try {
    $cols = $pdo->query('SHOW COLUMNS FROM clients')->fetchAll(PDO::FETCH_COLUMN);
    if (!in_array('data_nascimento', $cols)) {
        $pdo->exec("ALTER TABLE clients ADD COLUMN data_nascimento DATE DEFAULT NULL COMMENT 'Data de nascimento para promoção de aniversário'");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'client') {
    ...
    if ($id) {
        $stmt = $pdo->prepare('
            ...
            email=?,tags=?,observacoes=?,data_nascimento=?,updated_at=? WHERE id=? AND company_id=?
        ');
        $stmt->execute([
            $data['nome'], $data['telefone_principal'], $data['whatsapp'],
            $data['instagram_username'], $data['email'], $data['tags'],
            $data['observacoes'], $data['data_nascimento'], $nowUtc, $id, $companyId
        ]);
        ...
        $stmt = $pdo->prepare('
            ...
            email,tags,observacoes,data_nascimento,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)
        ');
        $stmt->execute([
            $companyId, $data['nome'], $data['telefone_principal'], $data['whatsapp'],
            $data['instagram_username'], $data['email'], $data['tags'],
            $data['observacoes'], $data['data_nascimento'], $nowUtc, $nowUtc
        ]);
...
if ($action === 'create' || ($action === 'edit' && $id)) {
    ...
    <style>
      ...
      /* ── Campo aniversário com destaque ── */
      .birthday-field-wrap {
        position: relative;
      }
      .birthday-field-wrap input {
        padding-left: 2.5rem !important;
        border-color: #fbbf24 !important;
        background: #fffbeb !important;
      }
      .birthday-field-wrap input:focus {
        border-color: #f59e0b !important;
        box-shadow: 0 0 0 3px rgba(251,191,36,.18) !important;
        background: #fff !important;
      }
      .birthday-field-wrap .bday-icon {
        position: absolute;
        left: .7rem;
        top: 50%;
...
if ($action === 'view' && $id) {
    ...
    // WhatsApp msg pré-definida para aniversário
    $waPromoMsg = '';
    if ($binfo['status'] !== 'none' && $waNumber) {
        $firstName = explode(' ', trim($client['nome']))[0];
        $waPromoMsg = 'https://wa.me/' . $waNumber . '?text=' . urlencode(
            'Oi, ' . $firstName . '! 🎂 Que dia especial! ' .
            'No seu aniversário, preparamos uma surpresa exclusiva pra você. ' .
            'Fala com a gente e garanta seu presente! 🎁'
        );
    }
    ...
    <div class="bday-banner today">
      <div>
        ...
        <p class="bday-banner-sub">Aproveite para enviar uma promoção exclusiva e fortalecer o relacionamento.</p>
      </div>
      <?php if ($waPromoMsg): ?>
        <a href="<?= sanitize($waPromoMsg) ?>" target="_blank" class="bday-promo-btn">
    ...
    <div class="bday-banner soon">
      <div>
        ...
        <p class="bday-banner-sub">Prepare uma oferta especial com antecedência para impactar no momento certo.</p>
      </div>
      <?php if ($waPromoMsg): ?>
        <a href="<?= sanitize($waPromoMsg) ?>" target="_blank" class="bday-promo-btn">
          🎁 Enviar promoção pelo WhatsApp
    ...
    <div class="bday-banner month">
      <div>
        ...
        <p class="bday-banner-sub">Ainda há tempo para preparar uma ação promocional especial.</p>
      </div>
    </div>
    <?php endif; ?>
...
$bdayFilter = trim($_GET['bday'] ?? ''); // filtro especial: 'week', 'month'
...
// Filtro por aniversário próximo (usando o mês/dia do campo data_nascimento)
if ($bdayFilter === 'week') {
    $where .= " AND data_nascimento IS NOT NULL AND (
        DATE_FORMAT(data_nascimento, '%m-%d') BETWEEN DATE_FORMAT(CURDATE(), '%m-%d') AND DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 7 DAY), '%m-%d')
        OR (MONTH(CURDATE()) = 12 AND DATE_FORMAT(data_nascimento, '%m-%d') <= DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 7 DAY), '%m-%d'))
    )";
...
// Contar aniversários próximos (7 dias) para o badge no filtro
$bdayWeekCount = 0;
$bdayMonthCount = 0;
try {
...
<style>
  ...
  /* Badge de aniversário na tabela */
...
<div class="cl-page">
  ...
  <div class="cl-stats">
    ...
    <div class="cl-stat">
      <p class="cl-stat-label">Aniversários este mês</p>
      <p class="cl-stat-val amber">🎂 <?= $bdayMonthCount ?></p>
    </div>
  </div>
  ...
  <div style="display:flex;flex-direction:column;gap:.65rem;">
    ...
    <div class="tag-filters">
      ...
      <a href="<?= sanitize(clients_url('bday=week' . ($search ? '&q=' . urlencode($search) : ''))) ?>"
         class="tag-filter-btn bday-btn <?= $bdayFilter === 'week' ? 'active' : '' ?>">
        🎂 Aniversário esta semana <span class="count"><?= $bdayWeekCount ?></span>
      </a>
      <?php endif; ?>
      <?php if ($bdayMonthCount > 0): ?>
      <a href="<?= sanitize(clients_url('bday=month' . ($search ? '&q=' . urlencode($search) : ''))) ?>"
         class="tag-filter-btn bday-btn <?= $bdayFilter === 'month' ? 'active' : '' ?>">
        🎉 Aniversariantes do mês <span class="count"><?= $bdayMonthCount ?></span>
      </a>
      <?php endif; ?>
    ...
    <?php else: ?>
          ...
          <tr class="<?= $rowClass ?>">
              ...
              <div class="cl-name-cell">
                ...
                <div>
                  <p class="cl-name">
                    ...
                    <?php if ($cBday['status'] === 'today'): ?>
                      <span style="font-size:.9rem;" title="Aniversário hoje!">🎂</span>
                    <?php elseif ($cBday['status'] === 'soon'): ?>
                      <span style="font-size:.85rem;opacity:.7;" title="Aniversário em <?= (int)$cBday['days'] ?> dias">🎈</span>
                    <?php endif; ?>
                  </p>
            ...
            <td>
              <?php if ($cBday['status'] !== 'none'): ?>
                <?php if ($cBday['status'] === 'today'): ?>
                  <span class="bday-badge today">🎂 Hoje!</span>
                <?php elseif ($cBday['status'] === 'soon'): ?>
                  <span class="bday-badge soon">🎈 em <?= (int)$cBday['days'] ?>d · <?= sanitize($cBday['label']) ?></span>
                <?php else: ?>
                  <span class="bday-badge normal">📅 <?= sanitize($cBday['label']) ?></span>
                <?php endif; ?>
              <?php else: ?>
                <span style="color:#cbd5e1;font-size:.8rem;">—</span>
              <?php endif; ?>
      ...
      <div class="cl-pagination">
        ...
        <div class="cl-pages">
          ...
          if (!in_array(1, $range)) {
              echo '<a class="page-btn" href="' . sanitize(clients_url($qs . 'page=1')) . '">1</a>';
              if ($range[0] > 2) echo '<span style="padding:0 .25rem;color:#94a3b8;">…</span>';
          }
          foreach ($range as $pg): ?>
          ...
          $last = end($range);
          if ($last < $totalPages) {
              if ($last < $totalPages - 1) echo '<span style="padding:0 .25rem;color:#94a3b8;">…</span>';
              echo '<a class="page-btn" href="' . sanitize(clients_url($qs . 'page=' . $totalPages)) . '">' . $totalPages . '</a>';
          } ?>
        </div>
        <?php endif; ?>
      </div>
...
async function runImport() {
  ...
  fd.append('form','csv_import'); fd.append('csv_file',file);
  try {
  ...
  } catch(e) { res.className='import-result error'; res.textContent='Erro de comunicação com o servidor.'; res.style.display='block'; }
  btn.disabled=false;
