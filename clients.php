...
if (file_exists(__DIR__ . '/agenda_helpers.php')) require_once __DIR__ . '/agenda_helpers.php';
...
function birthday_info(?string $dataNascimento, int $windowDays = 7): array {
    if (!$dataNascimento) return ['status' => 'none'];
    try {
        $today    = new DateTime('today');
        $birth    = new DateTime($dataNascimento);
        $thisYear = (int)$today->format('Y');
        $bDay     = new DateTime($thisYear . '-' . $birth->format('m-d'));
        if ($bDay < $today) $bDay->modify('+1 year');
        $diff  = (int)$today->diff($bDay)->days;
        $label = $birth->format('d/m');
        if ($diff === 0)           return ['status' => 'today',  'days' => 0,     'label' => $label];
        if ($diff <= $windowDays)  return ['status' => 'soon',   'days' => $diff, 'label' => $label];
        if ($diff <= 30)           return ['status' => 'month',  'days' => $diff, 'label' => $label];
        return                            ['status' => 'normal', 'days' => $diff, 'label' => $label];
    } catch (Throwable $e) { return ['status' => 'none']; }
}
...
try {
    ...
    if (!in_array('data_nascimento', $cols)) {
        $pdo->exec("ALTER TABLE clients ADD COLUMN data_nascimento DATE DEFAULT NULL");
    }
    if (!in_array('origem', $cols)) {
        $pdo->exec("ALTER TABLE clients ADD COLUMN origem VARCHAR(50) DEFAULT NULL");
    }
} catch (Throwable $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'client') {
    ...
    $nowUtc = now_utc();
    if ($id) {
        $pdo->prepare('UPDATE clients SET nome=?,telefone_principal=?,whatsapp=?,instagram_username=?,email=?,tags=?,observacoes=?,data_nascimento=?,updated_at=? WHERE id=? AND company_id=?')
            ->execute([$data['nome'],$data['telefone_principal'],$data['whatsapp'],$data['instagram_username'],$data['email'],$data['tags'],$data['observacoes'],$data['data_nascimento'],$nowUtc,$id,$companyId]);
        log_action($pdo,(int)$companyId,(int)$userId,'cliente_update','Cliente #'.$id);
        flash('success','Cliente atualizado com sucesso.');
    } else {
        $pdo->prepare('INSERT INTO clients (company_id,nome,telefone_principal,whatsapp,instagram_username,email,tags,observacoes,data_nascimento,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
            ->execute([$companyId,$data['nome'],$data['telefone_principal'],$data['whatsapp'],$data['instagram_username'],$data['email'],$data['tags'],$data['observacoes'],$data['data_nascimento'],$nowUtc,$nowUtc]);
...
/* ===========================
   ...
   =========================== */
if ($action === 'delete' && $id) {
    $pdo->prepare('DELETE FROM clients WHERE id=? AND company_id=?')->execute([$id,$companyId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'csv_import') {
    ...
    while (($row = fgetcsv($handle)) !== false) {
        ...
        try {
            ...
            if ($stmtChk->fetch()) { $stmtUpd->execute([$nome,'PDV_reativacao',now_utc(),$companyId,$phone]); $updated++; }
            else { $stmtIns->execute([$companyId,$nome,$phone,$phone,'PDV_reativacao','Importado via CSV.',gmdate('Y-m-d H:i:s',strtotime('-1 year')),gmdate('Y-m-d H:i:s',strtotime('-1 year'))]); $inserted++; }
...
if ($action === 'create' || ($action === 'edit' && $id)) {
    $client = ['nome'=>'','telefone_principal'=>'','whatsapp'=>'','instagram_username'=>'','email'=>'','tags'=>'','observacoes'=>'','data_nascimento'=>''];
    if ($id) {
    ...
    <style>
      ...
      .form-group input,.form-group textarea,.form-group select{width:100%;padding:.6rem .85rem;border:1.5px solid #e2e8f0;border-radius:8px;font-size:.875rem;color:#0f172a;background:#f8fafc;transition:border-color .15s,box-shadow .15s;outline:none;}
      ...
      .birthday-field-wrap{position:relative;}
      .birthday-field-wrap input{padding-left:2.5rem!important;border-color:#fbbf24!important;background:#fffbeb!important;}
      .birthday-field-wrap input:focus{border-color:#f59e0b!important;box-shadow:0 0 0 3px rgba(251,191,36,.18)!important;background:#fff!important;}
      .birthday-field-wrap .bday-icon{position:absolute;left:.7rem;top:50%;transform:translateY(-50%);font-size:1rem;pointer-events:none;line-height:1;}
      .birthday-hint-box{margin-top:.5rem;padding:.55rem .75rem;border-radius:8px;background:#fffbeb;border:1px solid #fde68a;font-size:.75rem;color:#92400e;display:flex;align-items:center;gap:.4rem;}
    </style>
    <div class="form-card">
      ...
      <form method="POST" class="form-grid">
        ...
        <div class="form-group">
          <label>Nome *</label>
          <input name="nome" value="<?= sanitize((string)($client['nome']??'')) ?>" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label>Tags</label>
          <input name="tags" value="<?= sanitize((string)($client['tags']??'')) ?>" placeholder="vip, recorrente, barbearia">
        </div>
        <div class="form-group form-span-2">
          <label>🎂 Data de Aniversário</label>
          <div class="birthday-field-wrap" style="max-width:280px;">
          ...
          <div class="birthday-hint-box" id="bday-hint-box" style="<?= empty($client['data_nascimento'])?'display:none;':'' ?>max-width:420px;">
            🎉 <span id="bday-hint-text">
              <?php if (!empty($client['data_nascimento'])):
                $binfo = birthday_info($client['data_nascimento']);
                if ($binfo['status']==='today') echo 'Aniversário é HOJE! 🎉';
                elseif ($binfo['status']==='soon') echo 'Aniversário em '.$binfo['days'].' dia'.($binfo['days']>1?'s':'').'! Prepare uma promoção.';
                elseif ($binfo['status']==='month') echo 'Aniversário em '.$binfo['days'].' dias.';
                else echo 'Data: '.date('d/m',strtotime($client['data_nascimento']));
              endif; ?>
            </span>
          </div>
          <p class="hint" style="margin-top:.4rem;">Usado para ativar promoção automática de aniversário.</p>
        </div>
        <div class="form-group form-span-2">
    ...
    <script>
    document.getElementById('f-nascimento').addEventListener('change', function() {
        const val = this.value;
        const box  = document.getElementById('bday-hint-box');
        const text = document.getElementById('bday-hint-text');
        ...
        if (diff===0)       text.textContent='Aniversário é HOJE! 🎉';
        else if (diff<=7)   text.textContent=`Aniversário em ${diff} dia${diff>1?'s':''}! Prepare uma promoção. (${label})`;
        else if (diff<=30)  text.textContent=`Aniversário em ${diff} dias. (${label})`;
        else                text.textContent=`Data registrada: ${label}`;
...
if ($action === 'view' && $id) {
    ...
    /* ── BARBEARIA: catálogo e agendamentos deste cliente ── */
    $servicesCatalog = [];
    try {
        if (function_exists('agenda_get_services_catalog')) {
    ...
    try {
        ...
        if (in_array('client_id', $apptCols)) {
            $aStmt = $pdo->prepare("
                SELECT a.*, b.name AS barber_name
                FROM appointments a
                LEFT JOIN barbers b ON b.id = a.barber_id AND b.company_id = a.company_id
                WHERE a.company_id = ? AND a.client_id = ?
                ORDER BY a.date DESC, a.time DESC
                LIMIT 30
            ");
            $aStmt->execute([$companyId, $id]);
            $allAppts = $aStmt->fetchAll(PDO::FETCH_ASSOC);
        }
    } catch(Throwable $e) {}

    $lastAppt     = $allAppts[0] ?? null;
    $daysSince    = null;
    if ($lastAppt) {
        $diff      = (new DateTime('today'))->diff(new DateTime($lastAppt['date']));
        $daysSince = abs((int)$diff->days);
    }
    $totalGasto   = array_sum(array_column($allAppts, 'total_price'));
    $totalVisitas = count($allAppts);
    ...
    $binfo = birthday_info($client['data_nascimento'] ?? null);
    $waPromoMsg = '';
    if ($binfo['status'] !== 'none' && $waNumber) {
        $firstName = explode(' ', trim($client['nome']))[0];
        $waPromoMsg = 'https://wa.me/'.$waNumber.'?text='.urlencode(
            'Oi, '.$firstName.'! 🎂 Que dia especial! No seu aniversário, preparamos uma surpresa exclusiva pra você. Fala com a gente e garanta seu presente! 🎁'
        );
    }
    ...
    <style>
      ...
      .interaction-form input,.interaction-form select,.interaction-form textarea{width:100%;padding:.55rem .8rem;border:1.5px solid #e2e8f0;border-radius:8px;font-size:.85rem;color:#0f172a;background:#f8fafc;outline:none;transition:border-color .15s;}
      ...
      /* Birthday banners */
      .bday-banner{border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;}
      .bday-banner.today{background:linear-gradient(135deg,#fef9c3,#fde68a);border:1.5px solid #f59e0b;}
      .bday-banner.soon{background:#fffbeb;border:1.5px solid #fcd34d;}
      .bday-banner.month{background:#f8fafc;border:1.5px solid #e2e8f0;}
      .bday-banner-text{font-size:.9rem;font-weight:600;color:#92400e;}
      .bday-banner-sub{font-size:.78rem;color:#b45309;margin-top:.15rem;}
      .bday-banner.month .bday-banner-text{color:#334155;}
      .bday-banner.month .bday-banner-sub{color:#64748b;}
      .bday-promo-btn{display:inline-flex;align-items:center;gap:.4rem;padding:.55rem 1rem;border-radius:8px;background:#16a34a;color:#fff;font-size:.82rem;font-weight:700;text-decoration:none;white-space:nowrap;}
      .bday-promo-btn:hover{background:#15803d;}
    </style>

    <!-- BANNER ANIVERSÁRIO -->
    <?php if ($binfo['status']==='today'): ?>
    <div class="bday-banner today">
      <div>
        <p class="bday-banner-text">🎂 HOJE É O ANIVERSÁRIO DE <?= strtoupper(sanitize(explode(' ',$client['nome'])[0])) ?>!</p>
        <p class="bday-banner-sub">Aproveite para enviar uma promoção exclusiva.</p>
      </div>
      <?php if ($waPromoMsg): ?>
        <a href="<?= sanitize($waPromoMsg) ?>" target="_blank" class="bday-promo-btn">🎁 Enviar promoção pelo WhatsApp</a>
      <?php endif; ?>
    </div>
    <?php elseif ($binfo['status']==='soon'): ?>
    <div class="bday-banner soon">
      <div>
        <p class="bday-banner-text">🎈 Aniversário em <?= (int)$binfo['days'] ?> dia<?= $binfo['days']>1?'s':'' ?> — <?= sanitize($binfo['label']) ?></p>
        <p class="bday-banner-sub">Prepare uma oferta especial com antecedência.</p>
      </div>
      <?php if ($waPromoMsg): ?><a href="<?= sanitize($waPromoMsg) ?>" target="_blank" class="bday-promo-btn">🎁 Enviar promoção pelo WhatsApp</a><?php endif; ?>
    </div>
    <?php elseif ($binfo['status']==='month'): ?>
    <div class="bday-banner month">
      <div>
        <p class="bday-banner-text">🗓️ Aniversário em <?= (int)$binfo['days'] ?> dias — <?= sanitize($binfo['label']) ?></p>
        <p class="bday-banner-sub">Ainda há tempo para preparar uma ação promocional.</p>
      </div>
    </div>
    <?php endif; ?>
      ...
      <div style="display:flex;gap:1rem;align-items:flex-start;flex:1;min-width:0;">
        ...
        <div class="client-meta">
          ...
          <div class="client-channels">
            ...
            <?php endif; ?>
            <?php if (!empty($client['data_nascimento'])): ?>
              <span class="channel-btn" style="background:#fffbeb;color:#b45309;cursor:default;">
                🎂 <?= sanitize(date('d/m',strtotime($client['data_nascimento']))) ?>
                <?php if ($binfo['status']==='today'): ?> — Hoje!
                <?php elseif ($binfo['status']==='soon'): ?> — em <?= (int)$binfo['days'] ?>d
                <?php endif; ?>
              </span>
            <?php endif; ?>
          </div>
    ...
    <!-- ══════════════════════════════════════════════════════════
         PAINEL DE REATIVAÇÃO BARBEARIA
    ══════════════════════════════════════════════════════════ -->
    <?php if ($daysSince !== null): ?>
    <?php
        $rBg  = $daysSince > 60 ? '#fef2f2'  : ($daysSince > 30 ? '#fefce8'  : '#f0fdf4');
        $rBd  = $daysSince > 60 ? '#fecaca'  : ($daysSince > 30 ? '#fef08a'  : '#bbf7d0');
        $rIco = $daysSince > 60 ? '🔴' : ($daysSince > 30 ? '🟡' : '🟢');
        if ($daysSince === 0)      $rMsg = 'Veio hoje! ✂️';
        elseif ($daysSince <= 7)   $rMsg = "Visita recente — há {$daysSince} dia" . ($daysSince > 1 ? 's' : '');
        elseif ($daysSince <= 30)  $rMsg = "Cliente ativo — última visita há {$daysSince} dias";
        elseif ($daysSince <= 60)  $rMsg = "Atenção — sem visita há {$daysSince} dias";
        else                       $rMsg = "Reativar — sem visita há {$daysSince} dias";
        $barbNameR = $lastAppt['barber_name'] ?? '';
        $svcSnapR  = '';
        if ($lastAppt && !empty($lastAppt['services_json'])) {
            $keys = json_decode($lastAppt['services_json'], true) ?: [];
            $lbls = array_map(fn($k) => (string)($servicesCatalog[$k]['label'] ?? $k), $keys);
            $svcSnapR = implode(', ', array_slice($lbls, 0, 2));
        }
    ?>
    <div style="background:<?= $rBg ?>;border:1px solid <?= $rBd ?>;border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;">
      <div style="display:flex;align-items:center;gap:.75rem;">
        <span style="font-size:1.5rem;"><?= $rIco ?></span>
        <div>
          <p style="font-size:.875rem;font-weight:700;color:#0f172a;">✂️ <?= $rMsg ?></p>
          <p style="font-size:.75rem;color:#64748b;margin-top:.15rem;">
            Último agendamento:
            <?= $lastAppt ? date('d/m/Y', strtotime($lastAppt['date'])) . ' às ' . substr((string)$lastAppt['time'], 0, 5) : '—' ?>
            <?php if ($barbNameR): ?> · <?= sanitize($barbNameR) ?><?php endif; ?>
            <?php if ($svcSnapR):  ?> · <em><?= sanitize($svcSnapR) ?></em><?php endif; ?>
            · <?= $totalVisitas ?> visita<?= $totalVisitas !== 1 ? 's' : '' ?>
            · total <?= format_currency($totalGasto) ?>
          </p>
        </div>
      </div>
      <?php if ($daysSince > 30 && !empty($client['whatsapp'])): ?>
      <?php
        $waReativNum = preg_replace('/\D/', '', $client['whatsapp']);
        $firstNameR  = explode(' ', trim((string)$client['nome']))[0];
        $waTxtR = "Olá {$firstNameR}! 👋\n\nFaz um tempo que você não passa na *Formen Barbearia*! ✂️\n\nTemos horários disponíveis essa semana. Que tal agendar?\n\n👉 https://bit.ly/Agenda_Barbearia";
        $waHrefR = "https://wa.me/{$waReativNum}?text=" . urlencode($waTxtR);
      ?>
      <a href="<?= sanitize($waHrefR) ?>" target="_blank"
         style="background:#25d366;color:#fff;padding:.55rem 1.1rem;border-radius:8px;font-size:.82rem;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:.4rem;white-space:nowrap;flex-shrink:0;">
    ...
    <div style="display:grid;grid-template-columns:1fr 300px;gap:1.25rem;align-items:start;">
      <div>
        ...
        <!-- ══════════════════════════════════════════════════════════
             HISTÓRICO DE AGENDAMENTOS BARBEARIA
        ══════════════════════════════════════════════════════════ -->
        <?php if (!empty($allAppts)): ?>
        <div class="section-card" style="margin-bottom:1.25rem;">
          <div class="section-card-header">
            <h3>✂️ Histórico Barbearia</h3>
            <span style="font-size:.75rem;color:#94a3b8;font-weight:500;"><?= $totalVisitas ?> visita<?= $totalVisitas !== 1 ? 's' : '' ?></span>
          </div>
          <!-- Mini KPIs -->
          <div style="display:grid;grid-template-columns:repeat(3,1fr);padding:.85rem 1.25rem;border-bottom:1px solid #f1f5f9;gap:.5rem;">
            <div style="text-align:center;">
              <p style="font-size:.65rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;">Visitas</p>
              <p style="font-size:1.5rem;font-weight:700;color:#6366f1;margin-top:.1rem;"><?= $totalVisitas ?></p>
            </div>
            <div style="text-align:center;">
              <p style="font-size:.65rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;">Total gasto</p>
              <p style="font-size:1.5rem;font-weight:700;color:#16a34a;margin-top:.1rem;"><?= format_currency($totalGasto) ?></p>
            </div>
            <div style="text-align:center;">
              <p style="font-size:.65rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;">Ticket médio</p>
              <p style="font-size:1.5rem;font-weight:700;color:#0f172a;margin-top:.1rem;"><?= format_currency($ticketMedio) ?></p>
            </div>
          </div>
          <!-- Lista -->
          <div style="padding:0 1.25rem;">
          <?php
          $stMap = ['agendado'=>['#dbeafe','#1d4ed8'],'concluido'=>['#dcfce7','#15803d'],'cancelado'=>['#fee2e2','#dc2626'],'faltou'=>['#fef9c3','#a16207']];
          foreach ($allAppts as $apt):
              $svcList = [];
              if (!empty($apt['services_json'])) {
                  foreach (json_decode($apt['services_json'],true)??[] as $k) {
                      $svcList[] = (string)($servicesCatalog[$k]['label'] ?? $k);
                  }
              }
              $st = $apt['status'] ?? 'agendado';
              [$stBg,$stFg] = $stMap[$st] ?? ['#f1f5f9','#475569'];
          ?>
          <div style="display:flex;align-items:center;gap:.85rem;padding:.75rem 0;border-bottom:1px solid #f8fafc;">
            <div style="width:36px;height:36px;border-radius:10px;background:#f5f3ff;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">✂️</div>
            <div style="flex:1;min-width:0;">
              <div style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;">
                <span style="font-size:.82rem;font-weight:700;color:#0f172a;">
                  <?= date('d/m/Y',strtotime($apt['date'])) ?> às <?= substr((string)$apt['time'],0,5) ?>
                </span>
                <span style="font-size:.63rem;font-weight:700;padding:.1rem .4rem;border-radius:20px;background:<?= $stBg ?>;color:<?= $stFg ?>;">
                  <?= strtoupper($st) ?>
                </span>
              </div>
              <p style="font-size:.78rem;color:#475569;margin-top:.1rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                <?= !empty($svcList) ? sanitize(implode(', ',$svcList)) : '<em style="color:#94a3b8;">Serviço não especificado</em>' ?>
                <?php if (!empty($apt['barber_name'])): ?> · <strong><?= sanitize($apt['barber_name']) ?></strong><?php endif; ?>
              </p>
            </div>
            <div style="text-align:right;flex-shrink:0;">
              <p style="font-size:.82rem;font-weight:700;color:#0f172a;"><?= format_currency((float)($apt['total_price']??0)) ?></p>
              <p style="font-size:.68rem;color:#94a3b8;"><?= (int)($apt['total_duration_minutes']??0) ?>min</p>
            </div>
          </div>
          <?php endforeach; ?>
          </div>
        ...
        <!-- HISTÓRICO DE ATENDIMENTOS (WhatsApp / CRM) -->
        <div class="section-card">
          ...
          <div class="timeline" style="padding:0 1.25rem;">
            <?php if (empty($interactions)): ?>
              <div style="padding:2rem 0;text-align:center;color:#94a3b8;font-size:.875rem;">Nenhum atendimento registrado ainda.</div>
            <?php endif; ?>
...
if ($tagFilter !== '') { $where .= ' AND tags LIKE ?'; $params[] = '%'.$tagFilter.'%'; }
if ($bdayFilter === 'week') {
    $where .= " AND data_nascimento IS NOT NULL AND DATE_FORMAT(data_nascimento,'%m-%d') BETWEEN DATE_FORMAT(CURDATE(),'%m-%d') AND DATE_FORMAT(DATE_ADD(CURDATE(),INTERVAL 7 DAY),'%m-%d')";
} elseif ($bdayFilter === 'month') {
    $where .= ' AND data_nascimento IS NOT NULL AND MONTH(data_nascimento)=MONTH(CURDATE())';
}
...
$bdayWeekCount = $bdayMonthCount = 0;
try {
    $bdayWeekCount  = (int)$pdo->query("SELECT COUNT(*) FROM clients WHERE company_id=$companyId AND data_nascimento IS NOT NULL AND DATE_FORMAT(data_nascimento,'%m-%d') BETWEEN DATE_FORMAT(CURDATE(),'%m-%d') AND DATE_FORMAT(DATE_ADD(CURDATE(),INTERVAL 7 DAY),'%m-%d')")->fetchColumn();
    $bdayMonthCount = (int)$pdo->query("SELECT COUNT(*) FROM clients WHERE company_id=$companyId AND data_nascimento IS NOT NULL AND MONTH(data_nascimento)=MONTH(CURDATE())")->fetchColumn();
} catch (Throwable $e) {}
...
$statsTotal = $total;
$statsRecent = $statsWa = $statsPdv = 0;
try {
    $statsRecent = (int)$pdo->query("SELECT COUNT(*) FROM clients WHERE company_id=$companyId AND created_at >= DATE_SUB(NOW(),INTERVAL 30 DAY)")->fetchColumn();
...
<style>
  ...
  .tag-filter-btn.bday-btn{border-color:#fde68a;color:#b45309;background:#fffbeb;}
  .tag-filter-btn.bday-btn:hover,.tag-filter-btn.bday-btn.active{border-color:#f59e0b;background:#fef3c7;color:#92400e;}
  .tag-filter-btn.bday-btn .count{background:#fde68a;color:#92400e;}
  ...
  .cl-table tbody tr.bday-today{background:linear-gradient(90deg,#fffbeb,#fff);}
  .cl-table tbody tr.bday-soon{background:linear-gradient(90deg,#fefce8,#fff);}
  ...
  .bday-badge{display:inline-flex;align-items:center;gap:.25rem;font-size:.7rem;font-weight:700;padding:.2rem .55rem;border-radius:20px;white-space:nowrap;}
  .bday-badge.today{background:#fef9c3;color:#b45309;}
  .bday-badge.soon{background:#fef3c7;color:#d97706;}
  .bday-badge.normal{background:#f1f5f9;color:#64748b;}
...
<div class="modal-overlay" id="csvModal">
  <div class="modal-box">
    ...
    <p>Arquivo <code style="background:#f1f5f9;padding:.1rem .4rem;border-radius:4px;font-size:.78rem;">clientes_whatsapp.csv</code>.<br>Chave única: <strong>WhatsApp + empresa</strong>.</p>
...
<div class="cl-page">
  ...
  <div class="cl-stats">
    <div class="cl-stat"><p class="cl-stat-label">Total</p><p class="cl-stat-val"><?= number_format($statsTotal,0,'.','.') ?></p></div>
    <div class="cl-stat"><p class="cl-stat-label">Novos (30d)</p><p class="cl-stat-val green"><?= $statsRecent ?></p></div>
    <div class="cl-stat"><p class="cl-stat-label">Com WhatsApp</p><p class="cl-stat-val indigo"><?= $statsWa ?></p></div>
    <div class="cl-stat"><p class="cl-stat-label">Aniversários este mês</p><p class="cl-stat-val amber">🎂 <?= $bdayMonthCount ?></p></div>
  </div>
  ...
  <div style="display:flex;flex-direction:column;gap:.65rem;">
    <form method="GET" class="cl-search-bar">
      ...
      <input type="hidden" name="bday" value="<?= sanitize($bdayFilter) ?>">
      <button type="submit" class="btn-primary" style="padding:.6rem 1rem;">Buscar</button>
      <?php if($search||$tagFilter||$bdayFilter): ?><a href="<?= sanitize(clients_url()) ?>" class="btn-outline" style="padding:.6rem 1rem;">Limpar</a><?php endif; ?>
    </form>
    <div class="tag-filters">
      <span style="font-size:.72rem;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:.05em;">Filtrar:</span>
      <?php if($bdayWeekCount>0): ?>
        <a href="<?= sanitize(clients_url('bday=week'.($search?'&q='.urlencode($search):''))) ?>"
           class="tag-filter-btn bday-btn <?= $bdayFilter==='week'?'active':'' ?>">🎂 Esta semana <span class="count"><?= $bdayWeekCount ?></span></a>
      <?php endif; ?>
      <?php if($bdayMonthCount>0): ?>
        <a href="<?= sanitize(clients_url('bday=month'.($search?'&q='.urlencode($search):''))) ?>"
           class="tag-filter-btn bday-btn <?= $bdayFilter==='month'?'active':'' ?>">🎉 Este mês <span class="count"><?= $bdayMonthCount ?></span></a>
      <?php endif; ?>
      <?php if(!empty($tagCounts)): ?>
        <span style="width:1px;height:16px;background:#e2e8f0;margin:0 .1rem;"></span>
        <a href="<?= sanitize(clients_url($search?'q='.urlencode($search):'')) ?>"
           class="tag-filter-btn <?= !$tagFilter&&!$bdayFilter?'active':'' ?>">Todos</a>
        <?php foreach(array_slice($tagCounts,0,8,true) as $tg=>$cnt): ?>
          <a href="<?= sanitize(clients_url('tag='.urlencode($tg).($search?'&q='.urlencode($search):''))) ?>"
             class="tag-filter-btn <?= $tagFilter===$tg?'active':'' ?>"><?= sanitize($tg) ?><span class="count"><?= $cnt ?></span></a>
        <?php endforeach; ?>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="cl-table-wrap">
    <?php if(empty($clients)): ?>
      <div class="cl-empty">
        ...
        <p><?= ($search||$tagFilter||$bdayFilter)?'Nenhum cliente encontrado para estes filtros.':'Nenhum cliente cadastrado ainda.' ?></p>
        <?php if(!$search&&!$tagFilter&&!$bdayFilter): ?>
    ...
    <?php else: ?>
      <table class="cl-table">
        ...
        <tbody>
          <?php foreach($clients as $c):
            ...
            $cBday   = birthday_info($c['data_nascimento']??null);
            $rowClass = match($cBday['status']){'today'=>'bday-today','soon'=>'bday-soon',default=>''};
          ?>
          <tr class="<?= $rowClass ?>">
            <td>
              <div class="cl-name-cell">
                ...
                <div>
                  <p class="cl-name">
                    <?= sanitize((string)$c['nome']) ?>
                    <?php if($cBday['status']==='today'): ?><span title="Aniversário hoje!">🎂</span>
                    <?php elseif($cBday['status']==='soon'): ?><span style="opacity:.7;" title="em <?= (int)$cBday['days'] ?> dias">🎈</span><?php endif; ?>
                  </p>
            ...
            <td>
              <?php $cIg=ltrim((string)($c['instagram_username']??''),'@'); if($cIg): ?>
            ...
            <td>
              <?php if(!empty($c['tags'])): foreach(array_filter(array_map('trim',explode(',',$c['tags']))) as $tg):
                $cls=str_contains(strtolower($tg),'pdv')?'pdv':(str_contains(strtolower($tg),'vip')?'vip':(str_contains(strtolower($tg),'inativo')?'inativo':''));
              ?><span class="tag-chip <?= $cls ?>"><?= sanitize($tg) ?></span><?php endforeach;
              else: ?><span style="color:#cbd5e1;font-size:.8rem;">—</span><?php endif; ?>
            </td>
            <td>
              <?php if($cBday['status']!=='none'): ?>
                <?php if($cBday['status']==='today'): ?><span class="bday-badge today">🎂 Hoje!</span>
                <?php elseif($cBday['status']==='soon'): ?><span class="bday-badge soon">🎈 em <?= (int)$cBday['days'] ?>d · <?= sanitize($cBday['label']) ?></span>
                <?php else: ?><span class="bday-badge normal">📅 <?= sanitize($cBday['label']) ?></span>
                <?php endif; ?>
              <?php else: ?><span style="color:#cbd5e1;font-size:.8rem;">—</span><?php endif; ?>
            </td>
      ...
      <div class="cl-pagination">
        <p class="cl-pagination-info">Exibindo <?= $offset+1 ?>–<?= min($offset+$perPage,$total) ?> de <?= number_format($total,0,'.','.') ?> clientes</p>
        <?php if($totalPages>1): ?>
        <div class="cl-pages">
          <?php
          $qs = ($search?'q='.urlencode($search).'&':'').($tagFilter?'tag='.urlencode($tagFilter).'&':'').($bdayFilter?'bday='.urlencode($bdayFilter).'&':'');
          ...
          foreach($range as $pg):?><a class="page-btn <?= $pg===$page?'active':'' ?>" href="<?= sanitize(clients_url($qs.'page='.$pg)) ?>"><?= $pg ?></a><?php endforeach;
          $last=end($range);if($last<$totalPages){if($last<$totalPages-1)echo '<span style="padding:0 .25rem;color:#94a3b8;">…</span>';echo '<a class="page-btn" href="'.sanitize(clients_url($qs.'page='.$totalPages)).'">'.$totalPages.'</a>';}?>
        </div>
        <?php endif; ?>
      </div>
    <?php endif; ?>
...
<script>
const zone=document.getElementById('uploadZone'),input=document.getElementById('csvInput'),fnWrap=document.getElementById('uploadFileName'),fnText=document.getElementById('uploadFileNameText');
...
function showFile(name){fnText.textContent=name;fnWrap.style.display='block';}
async function runImport(){
  const file=input.files[0];
  if(!file){alert('Selecione um arquivo CSV primeiro.');return;}
  const btn=document.getElementById('importBtn'),res=document.getElementById('importResult');
  btn.disabled=true;btn.textContent='Importando…';res.style.display='none';
  const fd=new FormData();fd.append('form','csv_import');fd.append('csv_file',file);
  try{
    const r=await fetch('clients.php',{method:'POST',body:fd});
    const json=await r.json();
    if(json.error){res.className='import-result error';res.textContent='Erro: '+json.error;}
    else{res.className='import-result success';res.innerHTML=`✓ Concluído — <strong>${json.inserted}</strong> inseridos · <strong>${json.updated}</strong> atualizados · <strong>${json.skipped}</strong> ignorados`;
    if(json.inserted>0||json.updated>0)setTimeout(()=>location.reload(),2000);}
    res.style.display='block';
  }catch(e){res.className='import-result error';res.textContent='Erro de comunicação.';res.style.display='block';}
  btn.disabled=false;btn.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg> Importar';
}
document.addEventListener('keydown',e=>{if(e.key==='Escape')document.getElementById('csvModal').classList.remove('open');});
</script>
