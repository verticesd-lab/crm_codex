...
// Força MySQL em UTC (se existir a função no helpers)
...
/**
 * Converte datetime vindo do banco (assumido UTC) para Cuiabá e formata.
 * - Se existir format_datetime_br() no helpers, usa.
 * - Senão, converte aqui com DateTimeZone.
 */
function safe_dt(?string $dt): string {
    if (!$dt) return '';

    // Se seu helpers tiver conversão (recomendado)
    if (function_exists('format_datetime_br')) {
        // tenta chamar com 2 args (dt, dbTz) se sua função suportar
        try {
            return format_datetime_br($dt, 'UTC');
        } catch (Throwable $e) {
            // fallback para versão com 1 arg
            try {
    ...
    // Fallback: trata como UTC e converte para America/Cuiaba
    try {
...
/**
 * Retorna "agora" em UTC para salvar no banco.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'client') {
    ...
    // WhatsApp: opcional (deixa como veio), mas se quiser pode forçar só dígitos:
    // $data['whatsapp'] = phone_digits($data['whatsapp']);
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                ...
                <div class="p-3 rounded-lg border border-slate-100 bg-slate-50">
                    ...
                    <p class="text-xl font-semibold">
                        <?= !empty($client['ultimo_atendimento_em']) ? safe_dt((string)$client['ultimo_atendimento_em']) : 'N/A' ?>
                    </p>
                </div>
            </div>
                    ...
                    <tbody>
                        <?php foreach ($interactions as $interaction): ?>
                            ...
                            <tr class="border-t border-slate-100">
                                ...
                                <td class="px-4 py-2 text-slate-600"><?= sanitize(safe_dt($interaction['created_at'] ?? null)) ?></td>
