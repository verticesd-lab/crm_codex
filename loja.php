...
$productsStmt = $pdo->prepare("SELECT * FROM products WHERE $where ORDER BY destaque DESC, created_at DESC");
$productsStmt->execute($params);
$products = $productsStmt->fetchAll();

$featuredStmt = $pdo->prepare('SELECT * FROM products WHERE company_id=? AND ativo=1 AND destaque=1 ORDER BY updated_at DESC LIMIT 5');
$featuredStmt->execute([$company['id']]);
$featured = $featuredStmt->fetchAll();

$categoriesStmt = $pdo->prepare('SELECT DISTINCT categoria FROM products WHERE company_id=? AND ativo=1 AND categoria IS NOT NULL AND categoria <> "" ORDER BY categoria');
$categoriesStmt->execute([$company['id']]);
$categories = $categoriesStmt->fetchAll(PDO::FETCH_COLUMN);

// Carrinho na sessão por empresa
...
if (isset($_GET['add'])) {
    ...
    header('Location: /loja.php?empresa=' . urlencode($slug));
    exit;
}
?>
<!DOCTYPE html>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center gap-4">
                    <?php if (!empty($company['logo'])): ?>
                        <img src="<?= sanitize($company['logo']) ?>" class="h-14 w-14 rounded-full border border-white/20 object-cover">
                    <?php else: ?>
                        <div class="h-14 w-14 rounded-full bg-brand-600 flex items-center justify-center text-lg font-semibold"><?= strtoupper(substr($company['nome_fantasia'],0,2)) ?></div>
                    <?php endif; ?>
                    <div>
                        ...
                        <?php if ($company['instagram_usuario']): ?>
                            <a class="text-sm text-sky-300 hover:underline" target="_blank" href="https://instagram.com/<?= ltrim(sanitize($company['instagram_usuario']), '@') ?>">Instagram</a>
                        <?php endif; ?>
                    </div>
                </div>
                <a href="/checkout.php?empresa=<?= urlencode($slug) ?>" class="inline-flex items-center gap-2 bg-emerald-500 text-slate-900 px-4 py-2 rounded-full shadow-lg shadow-emerald-500/30 hover:bg-emerald-400">
            ...
            <div class="relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-xl">
                ...
                <div class="relative p-6 flex flex-col md:flex-row gap-6">
                    <div class="flex-1 space-y-3">
                        ...
                        <p class="text-slate-200/80">Confira os produtos em destaque e aproveite as melhores condições direto pelo WhatsApp.</p>
                        <div class="flex flex-wrap gap-3">
                            <a class="px-5 py-2 rounded-full bg-brand-600 hover:bg-brand-700 font-semibold shadow-lg shadow-brand-600/30" href="#featured">Ver destaques</a>
                            <a class="px-5 py-2 rounded-full border border-white/20 hover:border-white/40" href="/promo.php?empresa=<?= urlencode($slug) ?>">Promoções</a>
                        </div>
                    </div>
                    <div class="w-full md:w-80">
                        <div class="relative h-48 rounded-xl border border-white/10 overflow-hidden" id="hero-carousel">
                            <?php foreach ($featured ?: array_slice($products,0,3) as $idx => $item): ?>
                                <div data-slide class="absolute inset-0 <?= $idx === 0 ? 'opacity-100' : 'opacity-0' ?> transition-opacity duration-700 ease-in-out bg-white/10 backdrop-blur flex flex-col justify-center p-4">
                                    <p class="text-xs uppercase tracking-wide text-emerald-200/80"><?= sanitize($item['categoria']) ?></p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <aside class="lg:col-span-1 bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 sticky top-4 h-fit">
                ...
                <div class="flex flex-col gap-2">
                    <a href="/loja.php?empresa=<?= urlencode($slug) ?>" class="px-3 py-2 rounded-lg border border-white/10 <?= $categoria === '' ? 'bg-brand-600 text-white' : 'hover:border-white/30' ?>">Todas</a>
                    <?php foreach ($categories as $cat): ?>
                        <a href="/loja.php?empresa=<?= urlencode($slug) ?>&categoria=<?= urlencode($cat) ?>" class="px-3 py-2 rounded-lg border border-white/10 <?= $categoria === $cat ? 'bg-brand-600 text-white' : 'hover:border-white/30' ?>"><?= sanitize($cat) ?></a>
                    <?php endforeach; ?>
                </div>
                <div class="pt-4 border-t border-white/10 space-y-2">
                    <p class="text-sm text-slate-200/80 font-semibold">Atendimento</p>
                    <a class="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-emerald-500 text-slate-900 font-semibold shadow hover:bg-emerald-400" href="https://api.whatsapp.com/send?phone=<?= urlencode($company['whatsapp_principal']) ?>&text=Ol%C3%A1%2C%20vim%20da%20loja%20online!">WhatsApp</a>
                    <?php if ($company['instagram_usuario']): ?>
                        <a class="inline-flex items-center gap-2 px-3 py-2 rounded-full border border-white/20 hover:border-white/40" target="_blank" href="https://instagram.com/<?= ltrim(sanitize($company['instagram_usuario']), '@') ?>">Instagram</a>
                    <?php endif; ?>
                </div>
            </aside>
            ...
            <div class="lg:col-span-3 space-y-6">
                <div class="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur">
                    <form class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <input name="q" value="<?= sanitize($search) ?>" placeholder="Buscar produto" class="rounded-lg border border-white/10 bg-white/5 text-white px-3 py-2 placeholder:text-slate-300">
                        <input name="categoria" value="<?= sanitize($categoria) ?>" placeholder="Categoria" class="rounded-lg border border-white/10 bg-white/5 text-white px-3 py-2 placeholder:text-slate-300">
                        <button class="rounded-lg px-4 py-2 bg-brand-600 hover:bg-brand-700 font-semibold">Filtrar</button>
                    </form>
                    <p class="mt-3 text-sm text-slate-200/80">Itens disponíveis agora. Adicione ao carrinho e finalize pelo WhatsApp.</p>
                </div>
                        ...
                        <?php foreach ($featured ?: array_slice($products,0,3) as $product): ?>
                            <div class="group bg-white/5 border border-white/10 rounded-xl shadow-lg hover:-translate-y-1 transition transform overflow-hidden">
                                <?php if ($product['imagem']): ?>
                                    <img src="<?= sanitize($product['imagem']) ?>" class="h-44 w-full object-cover group-hover:scale-105 transition">
                                <?php else: ?>
                                ...
                                <div class="p-4 space-y-2">
                                    ...
                                    <?php if (!empty($sizes)): ?>
                                        <div class="product-sizes">
                                            ...
                                            <?php foreach ($sizes as $size): ?>
                                                <span class="size-badge"><?= sanitize($size) ?></span>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php endif; ?>
                                    <?php
                                    ...
                                    <a href="/loja.php?empresa=<?= urlencode($slug) ?>&add=<?= $product['id'] ?>" class="inline-flex items-center justify-center w-full bg-brand-600 text-white px-4 py-2 rounded-lg hover:bg-brand-700 font-semibold">Adicionar ao carrinho</a>
                                </div>
                            </div>
                        <?php endforeach; ?>
