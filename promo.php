...
$slugEmpresa = $_GET['empresa'] ?? '';
$slugPromo = $_GET['promo'] ?? '';
if (!$slugEmpresa || !$slugPromo) { echo 'Promoção não encontrada.'; exit; }
...
$promoStmt = $pdo->prepare('SELECT * FROM promotions WHERE slug = ? AND company_id=? AND ativo=1');
$promoStmt->execute([$slugPromo, $company['id']]);
$promo = $promoStmt->fetch();
if (!$promo) { echo 'Promoção indisponível.'; exit; }
...
$mensagem = urlencode("Quero a oferta: {$promo['titulo']} da empresa {$company['nome_fantasia']} - " . (BASE_URL ? BASE_URL : '') . "/promo.php?empresa={$slugEmpresa}&promo={$slugPromo}");
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <div class="max-w-6xl mx-auto px-4 py-10 space-y-10">
        <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <?php else: ?>
                    <div class="h-12 w-12 rounded-full bg-brand-600 flex items-center justify-center text-lg font-semibold"><?= strtoupper(substr($company['nome_fantasia'],0,2)) ?></div>
                <?php endif; ?>
                <div>
            ...
            <div class="flex gap-3">
                <?php if ($company['instagram_usuario']): ?>
                    <a class="px-4 py-2 rounded-full border border-white/20 text-sm hover:border-white/40" target="_blank" href="https://instagram.com/<?= ltrim(sanitize($company['instagram_usuario']), '@') ?>">Instagram</a>
                <?php endif; ?>
                <a class="px-4 py-2 rounded-full bg-emerald-500 text-slate-900 font-semibold shadow-lg shadow-emerald-500/30 hover:bg-emerald-400" href="https://api.whatsapp.com/send?phone=<?= urlencode($company['whatsapp_principal']) ?>&text=<?= $mensagem ?>">Falar no WhatsApp</a>
            </div>
        </header>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
            <div class="space-y-6">
                ...
                <div class="flex flex-wrap gap-3">
                    <a class="px-6 py-3 rounded-full bg-brand-600 hover:bg-brand-700 font-semibold shadow-lg shadow-brand-600/30" href="https://api.whatsapp.com/send?phone=<?= urlencode($company['whatsapp_principal']) ?>&text=<?= $mensagem ?>">Quero essa oferta</a>
                    <a class="px-6 py-3 rounded-full border border-white/20 hover:border-white/40" href="tel:<?= urlencode($company['whatsapp_principal']) ?>">Ligar agora</a>
                </div>
                <div class="flex flex-wrap gap-4 text-sm text-slate-200/80">
                    <div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-emerald-400"></span> Atendimento humano ou WhatsApp</div>
                    <div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-emerald-400"></span> Estoque e agenda atualizados</div>
                    <div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-emerald-400"></span> Resposta rápida</div>
                </div>
            </div>
            <div class="relative">
                ...
                <div class="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl overflow-hidden">
                    ...
                    <?php else: ?>
                        <div class="w-full h-64 bg-gradient-to-r from-brand-600/60 to-emerald-500/60 flex items-center justify-center text-xl font-semibold">Oferta Especial</div>
                    <?php endif; ?>
                    <div class="p-6 space-y-3">
                        ...
                        <p class="text-sm text-slate-200/80">Clique para falar com nossa equipe e garanta sua condição especial.</p>
                        <a href="https://api.whatsapp.com/send?phone=<?= urlencode($company['whatsapp_principal']) ?>&text=<?= $mensagem ?>" class="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-emerald-500 text-slate-900 font-semibold shadow-md shadow-emerald-500/40 hover:bg-emerald-400">Falar com atendimento</a>
                    </div>
                </div>
            </div>
        </section>
        ...
        <footer class="pt-8 border-t border-white/10 text-sm text-slate-200/70">
            <p><?= sanitize($company['nome_fantasia']) ?> · Atendimento pelo WhatsApp · <?= sanitize($company['instagram_usuario']) ? '@'.sanitize($company['instagram_usuario']) : 'Siga no Instagram' ?></p>
        </footer>
    </div>
</body>
</html>
