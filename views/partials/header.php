    ...
    <div class="flex-1 flex flex-col">
        <header class="<?= $theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200' ?> border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            <div class="flex items-center gap-3">
                ...
                <?php else: ?>
                    <div class="h-10 w-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-semibold"><?= strtoupper(substr($_SESSION['company_name'] ?? 'A',0,2)) ?></div>
                <?php endif; ?>
                <div>
                    <p class="text-sm <?= $theme === 'dark' ? 'text-slate-400' : 'text-slate-500' ?>"><?= sanitize($_SESSION['company_name'] ?? 'Empresa') ?></p>
            ...
            <div class="flex items-center gap-3 relative">
                ...
                <a class="text-sm text-indigo-600 hover:underline" href="/loja.php?empresa=<?= sanitize($_SESSION['company_slug'] ?? '') ?>" target="_blank">Ver loja pública</a>
                <form action="/toggle-theme.php" method="POST" class="inline">
                ...
                <a class="text-sm <?= $theme === 'dark' ? 'text-slate-200' : 'text-slate-600' ?> hover:underline" href="/logout.php">Sair</a>
            </div>
        </header>
        <main class="p-6 flex-1">
<?php else: ?>
    <?php $loginBg = $GLOBALS['__LOGIN_BG__'] ?? ''; ?>
    <main class="min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden" style="<?= $loginBg ? 'background-image:url(' . sanitize($loginBg) . ');background-size:cover;background-position:center;background-repeat:no-repeat;' : '' ?>">
        <?php if ($loginBg): ?>
