...
// Garante que favicon/urls relativas não quebrem
function normalize_asset_url(string $url): string {
    $url = trim($url);
    if ($url === '') return '';
    ...
    // tenta montar com BASE_URL (se existir)
    if (defined('BASE_URL')) return rtrim(BASE_URL, '/') . '/' . ltrim($url, '/');

    return $url;
}

$faviconUrl = !empty($favicon) ? normalize_asset_url($favicon) : '';
?>
<!DOCTYPE html>
...
<head>
    ...
    <?php if (!empty($faviconUrl)): ?>
        <link rel="icon" href="<?= sanitize($faviconUrl) ?>" type="image/png">
    <?php endif; ?>
            ...
            <header class="<?= $theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200' ?> border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
                <div class="flex items-center gap-3">
                    <?php if ($logo = current_company_logo()): ?>
                        <img src="<?= sanitize($logo) ?>" class="h-10 w-10 rounded-full border <?= $theme === 'dark' ? 'border-slate-700' : 'border-slate-200' ?> object-cover" alt="Logo">
                    <?php else: ?>
